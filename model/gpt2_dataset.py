...
class GPT2Dataset(BaseDataset):
    def __init__(self, data_path, tokenizer, is_dev=False, mode='train', model_type='seq', max_pos_length=128, min_query_len=10, language='java'):
        super().__init__(data_path, tokenizer, is_dev, mode, max_pos_length=max_pos_length, min_query_len=min_query_len, model_type=model_type, language=language)
    ...
    def tokenize_and_split(self, examples):
        tokenized = self.tokenizer(examples['code'],padding='max_length',max_length=self.max_pos_length+self.min_query_len, truncation=True)
        new_input_ids = []
        answers = []
        new_input_code = []
        answer_code = []
